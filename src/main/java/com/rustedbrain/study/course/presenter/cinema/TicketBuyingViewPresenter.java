...
public class TicketBuyingViewPresenter implements TicketsBuyingView.TicketBuyViewListener, Serializable {
    ...
    public void entered(ViewChangeListener.ViewChangeEvent event) {
        ...
        List<Long> seatIds = Arrays.stream(parameterMap.get(SEATS_ID_PARAM_KEY)
        ...
        if (authenticationService.isAuthenticated() && UserRole.MEMBER.equals(authenticationService.getUserRole())) {
            ...
            this.view.showFilmScreeningEvent(filmScreeningEvent, seats, member.getName(), member.getSurname(), member.getEmail());
        } else {
        ...
        if (name == null || name.isEmpty()) {
            ...
            try {
                if (authenticationService.isAuthenticated()) {
                    List<TicketInfo> tickets = cinemaService.reserveTickets(name, surname, authenticationService.getUserLogin(), filmScreeningEvent, seats);
                    logger.info("TicketInfo's successfully reserved: " + tickets);
                    new PageNavigator().navigateToTicketsInfoView(tickets.stream().mapToLong(TicketInfo::getId).toArray());
                } else {
