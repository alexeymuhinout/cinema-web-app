...
public class AdminProfileEditTab extends ProfileEditTab {
    ...
    public AdminProfileEditTab(List<ProfileView.ViewListener> listeners, User currUser, List<User> users, List<City> cities) {
    ...
    private Layout createUserSelectionLayout() {
        ...
        Button showMeButton = new Button("Me", (Button.ClickListener) event -> listeners.forEach(viewListener -> viewListener.buttonEditShowMeClicked(users)));
    ...
    private Layout createBlockUnblockControlPanel() {
        Button blockButton = new Button("Block", (Button.ClickListener) event -> listeners.forEach(viewListener -> viewListener.buttonBlockClicked(currUser.getId())));
        Button unblockButton = new Button("Unblock", (Button.ClickListener) event -> listeners.forEach(viewListener -> viewListener.buttonUnblockClicked(currUser.getId())));
