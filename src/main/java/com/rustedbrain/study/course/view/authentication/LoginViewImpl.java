...
public class LoginViewImpl extends VerticalLayout implements LoginView {
    ...
    private Panel getLoginPanel() {
        ...
        content.addComponent(new Button("Login", (Button.ClickListener) event -> loginViewListeners.forEach(loginViewListener -> loginViewListener.loginButtonClicked(loginTextField.getValue(), passwordTextField.getValue(), checkBoxRemember.getValue()))));
